...
var adult, adultIncomeTemplate, baseURL, changeHouseHoldMembers, child, childIncomeTemplate, currentPanel, data, definitionOn, downloadPDF, formatPDF, forwardPanelTransition, generatePDF, hasSSN, householdMembers, identityPanelIndex, objectForFields, panel, panelStack, pdfDocument, populateAdultIncome, populateChildrenIncome, processAdultInfo, processChildrenInfo, processEligibilityInfo, processIdentityInfo, processIncomeInfo, processParentInfo, processProgramInfo, processSSNInfo, processSubmit, programParticipant, setUpButtons, setUpChildPanel, setUpDefinitions, setUpHouseholdPanel, setUpIdentityPanel, setUpIncomePanel, setUpProgramPanel, setUpSSNPanel, setUpSubmitPanel, setUpValidation, setUpWindow, showNextPanel, showPreviousPanel, skipToIdentityPanel, submitForm, submitPanelIndex, submitted, updateProgressBar, _ref;
...
downloadPDF = function() {
  return pdfDocument.download('Reduced School Lunch Application');
};
...
generatePDF = function() {
  ...
  return pdfDocument.getDataUrl(function(result) {
    return $('#pdfView').attr('src', result);
  });
};
...
setUpWindow = function() {
  ...
  return $(window).on('beforeunload', function(e) {
    if (!submitted) {
      return 'You have not submitted your form yet. If you leave the page now, your data will be lost.';
    }
  });
};
...
setUpButtons = function() {
  ...
  return $('#pdfDownload').click(downloadPDF);
};
...
setUpDefinitions = function(parent) {
  ...
  $(parent).find('.has-definition').on('focusin mouseenter', function() {
    ...
    return $('#status-card').css('display', 'block').stop(true).animate({
      opacity: 1
    });
  });
  return $(parent).find('.has-definition').on('focusout mouseleave', function() {
    definitionOn = false;
    return $('#status-card').stop(true).animate({
      opacity: 0
    }, 400, "swing", function() {
      if (!definitionOn) {
        return $(this).css('display', 'none');
